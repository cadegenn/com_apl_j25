...
/**
 * aplModelImportV2AdherentsProfessions List Model
 */
class aplModelImportV2AdherentsProfessions extends JModelList
{
    /*
	...
	public function __construct($config = array()) {
		...
		/*if (!$this->database || $this->database=="") { JFactory::getApplication()->enqueueMessage(JText::_('COM_APL_NO_IMPORT_DATABASE'), 'error'); return; }
		if (!$this->db_host || $this->db_host=="") { JFactory::getApplication()->enqueueMessage(JText::_('COM_APL_NO_IMPORT_DBHOST'), 'error'); return; }
		if (!$this->db_username || $this->db_username=="") { JFactory::getApplication()->enqueueMessage(JText::_('COM_APL_NO_IMPORT_DBUSERNAME'), 'error'); return; }
		if (!$this->db_passwd || $this->db_passwd=="") { JFactory::getApplication()->enqueueMessage(JText::_('COM_APL_NO_IMPORT_DBPASSWD'), 'error'); return; }
		 * 
		 */
	...
	public function import($cid) {
		...
		foreach ($cid as $key => $id) {
			...
			$query_apl->select('professions.*')->from($this->database.'.professions')->where('professions.id = '.$id);
			...
			foreach( $rows as $row ) {
				...
				$query->delete('#__apl_adherents_professions')->where('id = '.$profession->id);
				$db->setQuery($query);
				$db->execute();
				// on insert le nouvel objet (qui en fait contient les anciennes data de la V2 du site)
				$db->insertObject('#__apl_adherents_professions', $profession );
			}
		}
