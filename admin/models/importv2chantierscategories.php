...
/**
 * aplModelImportV2ChantiersCategories List Model
 */
class aplModelImportV2ChantiersCategories extends JModelList
{
    /*
    ...
    protected function getListQuery()
    {
		...
		$query->from($this->database.'.categories_chantier');
	...
	public function import($cid) {
		...
		foreach ($cid as $key => $id) {
			...
			$query_apl->select('categories_chantier.*')->from($this->database.'.categories_chantier')->where('categories_chantier.id = '.$id);
			...
			foreach( $rows as $row ) {
				...
				$categorie = clone $row; //new stdClass();//$row;
				// on ajoute les nouveaux champs
				$categorie->nom = "";
				$categorie->published = 0;//($row->obsolete ? 2 : $categorie->published);
				$categorie->mapType = 0;
				// on formatte les données correctement
				$categorie->nom = $row->text;
				$categorie->mapGlat = $row->glat;
				$categorie->mapGlng = $row->glng;
				$categorie->published = ($row->enable ? 1 : $categorie->published);
				$categorie->mapType = $row->map_type;
				// on supprime les attributs de trop
				unset($categorie->text);
				unset($categorie->glat);
				unset($categorie->glng);
				unset($categorie->map_type);
				unset($categorie->enable);
				//echo("<pre>".var_dump($categorie)."</pre>");
				// on supprime les id en conflit
				$query->delete('#__apl_chantiers_categories')->where('id = '.$categorie->id);
				$db->setQuery($query);
				$db->execute();
				// on insert le nouvel objet (qui en fait contient les anciennes data de la V2 du site)
				$db->insertObject('#__apl_chantiers_categories', $categorie );
			}
		}
